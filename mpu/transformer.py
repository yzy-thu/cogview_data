...
class GPT2ParallelSelfAttention(torch.nn.Module):
    ...
    def forward(self, hidden_states, ltor_mask, position_embeddings=None, r_w_bias=None, r_r_bias=None, mem=None):
        ...
        attention_scores = torch.mul(attention_scores, ltor_mask / math.sqrt(self.hidden_size_per_attention_head)) - \
...
class GPT2ParallelTransformer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if relative_encoding:
            ...
            # A B block embedding # TODO is this necessary?
    ...
    def forward(self, hidden_states, position_ids, attention_mask, *mems):
        ...
        # if sep == 0:
        #     hidden_states = hidden_states + self.type_embedding[1]
        # elif sep == hidden_states.shape[1]:
        #     hidden_states = hidden_states + self.type_embedding[0]
        # else:
        #     hidden_states = hidden_states + torch.cat((self.type_embedding[0].expand(int(sep), -1), self.type_embedding[1].expand(query_length - int(sep), -1)))
