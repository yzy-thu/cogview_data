...
class LayerNorm(FusedLayerNorm):
    ...
    def forward(self, x):
        return super().forward(x / (x.max().detach()/8))
...
class GPT2ParallelTransformerLayer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # self.input_layernorm.weight.data /= 4 # rebalance layernorm
        ...
        # self.post_attention_layernorm.weight.data /= 2 # rebalance layernorm
        self.scale_normalization = scale_normalization
        if scale_normalization:
            self.third_layernorm = LayerNorm(hidden_size,
                                                    eps=layernorm_epsilon)
            # self.third_layernorm.weight.data /= 8 # rebalance layernorm
            self.fourth_layernorm = LayerNorm(hidden_size,
                                                    eps=layernorm_epsilon)
            # self.fourth_layernorm.weight.data /= 8 # rebalance layernorm
    ...
    def forward(self, hidden_states, ltor_mask, pivot_idx=None, is_sparse=0, mem=None):
        ...
        if self.fout is not None:
            # self.fout.write(f'layernorm output1: {layernorm_output1.max()}\nlayernorm_output2: {layernorm_output.max()} \nlayernorm_output3: {mlp_output.max()}\n')
            self.fout.write(f'hidden1: {layernorm_input.max()}\nhidden2: {output.max()}\n')
...
class GPT2ParallelTransformer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # torch.nn.init.normal_(self.position_embeddings.weight, mean=0.0, std=init_method_std/math.sqrt(2*num_layers))
        ...
        # self.final_layernorm.weight.data /= 32 # rebalance layernorm
    ...
    def forward(self, hidden_states, position_ids, attention_mask, txt_indices_bool, img_indices_bool, is_sparse=0, *mems):
        ...
        if self.checkpoint_activations:
            ...
            while l < num_layers:
                ...
                if fout is not None:
                    fout.write(f'Layer {l} input: max {hidden_states.max()} mean {hidden_states.min()} var {hidden_states[0].var()}\n')

                # if l == num_layers - 2 and dist.get_rank() == 0:
        ...
        # print(f'Layer {l} input: max {hidden_states.max()} mean {hidden_states.min()} var {hidden_states[0].var()}\n')
        if fout is not None:
            fout.close()
            for layer in self.layers:
                layer.fout = None
