...
def forward_step(data_iterator, model, args, timers, mems):
    ...
    txt_loss = losses[txt_indices_bool].detach().sum() / max(txt_indices_bool.sum(), 1)
...
def train_step(data_iterator, model, optimizer, lr_scheduler,
    ...
    while True:
        ...
        # =====================   Logs   ======================== #
        # if dist.get_rank() == 0:
        #     with open('parameter_grad.txt', 'w') as fout:
        #         for name, p in model.named_parameters():
        #             fout.write(f'{name} value: max {p.abs().max()} mean {p.abs().mean()}\n')
        #             if p.grad is not None:
        #                 # if p.grad.isnan().any():
        #                 #     import pdb; pdb.set_trace()
        #                 fout.write(f'{name} grad: max {p.grad.abs().max()} mean {p.grad.abs().mean()}\n')
        #                 if name == 'module.module.word_embeddings.weight':
        #                     t = p.grad.abs().max(dim=-1)[0].max(dim=-1)[1].view(-1)
        #                     freq = [(_tokens[i]==idx).sum().item() for i, idx in enumerate(t)]
        #                     fout.write(f'{name} maxgrad token_id: {t} freq: {freq}\n')
        #                     # if freq[0] == 0:
        #                     #     import pdb; pdb.set_trace()
        #             fout.write('\n')
        #     os.chmod("parameter_grad.txt",stat.S_IRWXU|stat.S_IRWXG|stat.S_IRWXO)
...
def report_iteration_metrics(summary_writer, optimizer, lr, loss, elapsed_time, step, total_step, args, img_loss, txt_loss):
    ...
    log_string += ' txt loss {:.6E} |'.format(txt_loss)
    if args.fp16:
